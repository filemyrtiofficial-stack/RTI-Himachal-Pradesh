 ...
 * 
 * Special Handling:
 * - Himachal Pradesh: PDFs stored in public folder (organized in subfolders)
...
export const PDFDownloadModal: React.FC<PDFDownloadModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // The pdfPath from getPDFPath() is now just the filename (e.g., "RTI_Maharashtra_Finance_Department.pdf")
      // All PDFs are directly in /assets/PDF/ folder
      ...
      try {
        // Extract state name from department name for API call
        const stateMatch = departmentName.match(/RTI\s+([^D]+?)(?:\s+Department|$)/i);
        const stateName = stateMatch ? stateMatch[1].trim().toLowerCase().replace(/\s+/g, '-') : 'maharashtra';
        
        const apiUrl = `${apiBaseUrl}/api/v1/pdf/${stateName}/${encodeURIComponent(pdfPath)}`;
        ...
        if (response && response.ok) {
          ...
          const downloadFilename = departmentName
            .replace(/RTI\s+(Delhi|Telangana|Himachal Pradesh)\s+/gi, '')
            .replace(/\s+/g, '_')
            .concat('.pdf');
      ...
      // Approach 2: Try public folder (PDFs are directly in /assets/PDF/)
      if (!downloadSuccess) {
        try {
          // PDFs are directly in /assets/PDF/ with just the filename
          const publicPath = `/assets/PDF/${pdfPath}`;

          // Open PDF directly from public folder URL
